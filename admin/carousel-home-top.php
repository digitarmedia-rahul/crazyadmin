    ...
    <div class="wrapper">
        ...
        if (isset($_POST['addNew'])) {
            ...
            if ($_FILES['newicon']['name']) {
                ...
                $newFileName = "co_" . time() . "." . $fileExtension;
            ...
            $q = 'INSERT INTO carousel_home_top (carousel_name, carousel_url, tracking_url, description) VALUES ("' . trim($_POST['name']) . '", "' . $filePath . '", "' . trim($_POST['redirect']) . '", "' . trim($_POST['desc']) . '")';
            $mysqli->query($q);
            $mysqli->close();
            header('Location: ../admin/carousel-home-top.php');
                                ...
                                <div class="panel-body">
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-striped dataTable" id="userListDetails">
                                            <tbody>
                                                <tr>
                                                    <th colspan="3" class="text-center">Fill the Carousel Details</th>
                                                </tr>
                                                <form action="" method="post" enctype="multipart/form-data">
                                                    <tr>
                                                        <th style="width: 200px !important" class="text-center">Carousel</th>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="text-center"> <input accept=".jpg, .jpeg, .png" class="form-control" style="width: 300px !important" type="file" name="newicon" id="newicon" required /> </td>
                                                    </tr>
                                                    <tr>
        ...
        <?php
            ...
            $is_avail = $mysqli->query("SELECT * FROM `carousel_home_top` WHERE id ='$carouselId'");

            if ($is_avail->num_rows > 0) {
                if ($filePath != '') {
                    $q = 'update carousel_home_top set carousel_url	 = "' . $filePath . '" where id = "' . $carouselId . '" ';
                    $mysqli->query($q);
                } else {
                    if (isset($_POST['name']) && trim($_POST['name']) != '') {
                        $q = 'update carousel_home_top set carousel_name = "' . trim($_POST['name']) . '" where id = "' . $carouselId . '" ';
                        $mysqli->query($q);
                    }
                    if (isset($_POST['desc']) && trim($_POST['desc']) != '') {
                        $q = 'update carousel_home_top set description = "' . trim($_POST['desc']) . '" where id = "' . $carouselId . '" ';
                        $mysqli->query($q);
                    }
                    if (isset($_POST['redirect']) && trim($_POST['redirect']) != '') {
                        $q = 'update carousel_home_top set tracking_url = "' . trim($_POST['redirect']) . '" where id = "' . $carouselId . '" ';
                        $mysqli->query($q);
                    }
                    if (isset($_POST['status']) && trim($_POST['status']) != '') {
                        $q = 'update carousel_home_top set status = "' . trim($_POST['status']) . '" where id = "' . $carouselId . '" ';
                        $mysqli->query($q);
                    }
                }
            }
            ...
            $deviceid = $mysqli->query("SELECT * FROM `carousel_home_top` WHERE status =1 AND id =" . trim($_GET['com']));
            $carouselData = $deviceid->fetch_assoc();
                                ...
                                <div class="panel-body">
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-striped dataTable" id="userListDetails">
                                            <tbody>
                                                <tr>
                                                    <th colspan="3" class="text-center">Carousel Details</th>
                                                </tr>
                                                <form action="" method="post" enctype="multipart/form-data">
                                                    <input type="hidden" name="carouselId" value="<?= $_GET['com'] ?>">
                                                    <tr>
                                                        <th style="width: 200px !important" class="text-center">Carousel</th>
                                                        <td class="text-center"><?= $carouselData['carousel_name'] ?></td>
                                                    ...
                                                    <tr>
                                                        <th style="width: 200px !important" class="text-center">Redirect Link</th>
                                                        <td class="text-center"><?= $carouselData['tracking_url'] ?></td>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="text-center"><?= ($carouselData['status'] == '1' ? 'Active' : 'Deactive') ?></td>
                                                        <td style="width: 300px !important" class="text-center"><select name="status" class="form-control" id="">
                                                                <option value="1" <?= ($carouselData['status'] == '1' ? 'selected' : '') ?>>Active</option>
                                                                <option value="0" <?= ($carouselData['status'] == '0' ? 'selected' : '') ?>>Deactive</option>
                                                            </select></td>
                                                    </tr>
                                                    <tr>
                                                        <th style="width: 200px !important" class="text-center">Image</th>
                                                        <td class="text-center"> <?php
                                                                                    if ($carouselData['carousel_url']) {
                                                                                    ?>
                                                                <img src="<?= $carouselData['carousel_url'] ?>" width="
                                                        50" height="50">
                                                            <?php
                                                                                    }
                                                            ?>
                                                        </td>
                                                        <td class="text-center"> <input class="form-control" style="width: 300px !important" accept=".jpg, .jpeg, .png" type="file" name="newicon" id="newicon" /> </td>
                                                    </tr>
                                                    <tr>
                                                        <th style="width: 200px !important" class="text-center">Desc</th>
                                                        <td class="text-center"><?= ($carouselData['description'] ? $carouselData['description'] : 'NA') ?></td>
                                                        <td class="text-center"><textarea class="form-control" name="desc" id="" cols="25" rows="2"></textarea></td>
                                                    </tr>
                                                    <tr>
        ...
        <?php
            ...
            $deviceid = $mysqli->query("SELECT * FROM `carousel_home_top` WHERE $sqlq ORDER BY id DESC");
            while ($deviceids = $deviceid->fetch_assoc()) {
                $carouselData[] =  $deviceids;
            }
                                        ...
                                        <table class="table table-bordered table-striped dataTable" id="userListDetails">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="text-center">Carousel</th>
                                            ...
                                            <tbody>
                                                <?php
                                                foreach ($carouselData as $k => $data) {
                                                ?>
                                                    <tr>
                                                        <td class="text-center"><?= ($k + 1) ?></td>
                                                        <td class="text-center"><?= $data['carousel_name'] ?></td>
                                                        <td class="text-center"><?php
                                                                                if ($data['carousel_url']) {
                                                                                ?>
                                                                <img src="<?= $data['carousel_url'] ?>" width="
                                                        50" height="50">
                                                            <?php
                                                                                }
                                                            ?>
