...
<body class="skin-blue sidebar-mini fixed" data-spy="scroll" data-target="#scrollspy">
    <div class="wrapper">
        ...
        if (isset($_POST['addNew'])) {
            ...
            $deviceid = $mysqli->query("SELECT priority FROM `top_brand_list` ORDER BY id DESC LIMIT 1");
            $brandData = $deviceid->fetch_assoc();
            $priority = $brandData['priority'] ? $brandData['priority'] : 0;
            $priority++;
            $q = 'INSERT INTO top_brand_list (brand_name, image_url, tracking_url, description, priority) VALUES ("' . trim($_POST['name']) . '", "' . $filePath . '", "' . trim($_POST['redirect']) . '", "' . trim($_POST['desc']) . '", ' . $priority . ')';
            $mysqli->query($q);
            $mysqli->close();
            header('Location: ../admin/top-brand-home.php');
                                ...
                                <div class="panel-body">
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-striped dataTable" id="userListDetails">
                                            <tbody>
                                                <tr>
                                                    <th colspan="3" class="text-center">Fill the Brand Details</th>
                                                </tr>
                                                <form action="" method="post" enctype="multipart/form-data">
                                                    <tr>
                                                        <th style="width: 200px !important" class="text-center">Brand</th>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="text-center"> <input class="form-control" style="width: 300px !important" accept=".jpg, .jpeg, .png" type="file" name="newicon" id="newicon" required /> </td>
                                                    </tr>
                                                    <tr>
        ...
        <?php
            ...
            $is_avail = $mysqli->query("SELECT * FROM `top_brand_list` WHERE id ='$carouselId'");

            if ($is_avail->num_rows > 0) {
                if ($filePath != '') {
                    $q = 'update top_brand_list set image_url	 = "' . $filePath . '" where id = "' . $carouselId . '" ';
                    $mysqli->query($q);
                } else {
                    if (isset($_POST['name']) && trim($_POST['name']) != '') {
                        $q = 'update top_brand_list set brand_name = "' . trim($_POST['name']) . '" where id = "' . $carouselId . '" ';
                        $mysqli->query($q);
                    }
                    if (isset($_POST['desc']) && trim($_POST['desc']) != '') {
                        $q = 'update top_brand_list set description = "' . trim($_POST['desc']) . '" where id = "' . $carouselId . '" ';
                        $mysqli->query($q);
                    }
                    if (isset($_POST['redirect']) && trim($_POST['redirect']) != '') {
                        $q = 'update top_brand_list set tracking_url = "' . trim($_POST['redirect']) . '" where id = "' . $carouselId . '" ';
                        $mysqli->query($q);
                    }
                    if (isset($_POST['status']) && trim($_POST['status']) != '') {
                        $q = 'update top_brand_list set status = "' . trim($_POST['status']) . '" where id = "' . $carouselId . '" ';
                        $mysqli->query($q);
                    }
                }
            }
            ...
            $deviceid = $mysqli->query("SELECT * FROM `top_brand_list` WHERE status =1 AND id =" . trim($_GET['com']));
            $brandData = $deviceid->fetch_assoc();
                                ...
                                <div class="panel-body">
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-striped dataTable" id="userListDetails">
                                            <tbody>
                                                <tr>
                                                    <th colspan="3" class="text-center">Brand Details</th>
                                                </tr>
                                                <form action="" method="post" enctype="multipart/form-data">
                                                    <input type="hidden" name="carouselId" value="<?= $_GET['com'] ?>">
                                                    <tr>
                                                        <th style="width: 200px !important" class="text-center">Brand</th>
                                                        <td class="text-center"><?= $brandData['brand_name'] ?></td>
                                                    ...
                                                    <tr>
                                                        <th style="width: 200px !important" class="text-center">Redirect Link</th>
                                                        <td class="text-center"><?= $brandData['tracking_url'] ?></td>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="text-center"><?= ($brandData['status'] == '1' ? 'Active' : 'Deactive') ?></td>
                                                        <td style="width: 300px !important" class="text-center"><select name="status" class="form-control" id="">
                                                                ...
                                                                <option value="0" <?= ($brandData['status'] == '0' ? 'selected' : '') ?>>Deactive</option>
                                                            </select></td>
                                                    </tr>
                                                    <tr>
                                                        ...
                                                        <td class="text-center"> <?php
                                                                                    ...
                                                                                    ?>
                                                                <img src="<?= $brandData['image_url'] ?>" width="
                                                        50" height="50">
                                                            <?php
                                                                                    }
                                                            ?>
                                                    ...
                                                    <tr>
                                                        <th style="width: 200px !important" class="text-center">Desc</th>
                                                        <td class="text-center"><?= ($brandData['description'] ? $brandData['description'] : 'NA') ?></td>
                                                        <td class="text-center"><textarea class="form-control" name="desc" id="" cols="25" rows="2"></textarea></td>
                                                    </tr>
                                                    <tr>
        ...
        <?php
            ...
            $deviceid = $mysqli->query("SELECT * FROM `top_brand_list` ORDER BY id DESC");
